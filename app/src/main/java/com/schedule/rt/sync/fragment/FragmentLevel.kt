...
import com.schedule.rt.sync.databinding.FragmentLevelBinding
...
class FragmentLevel(
    ...
    private fun recyclerView() {
        ...
        val adapterLevel = AdapterLevel(
            btnFirst = false,
            btnSecond = false,
            btnNext = true,
            onNextClick = {
                vmClasses.uidLevelReference(it.uidLevel)
                vmData.sendUidLevel(it.uidLevel.toString())
                replaceFragmentWithBackStack(
                    fragmentContainer,
                    FragmentClasses(fragmentContainer, toastContainer), null
                )
            }
        )
        recyclerView.adapter = adapterLevel

        vmLevel.getLevel().observe(viewLifecycleOwner) {
            adapterLevel.updateData(it)

...
import com.schedule.rt.sync.databinding.FragmentNameBinding
...
class FragmentName(
    ...
    fun inputName() {
        ...
        val etName = binding.etName.text.toString().capitalizeEachWord().capitalizeAfterDot()
        if (etName.isNotEmpty()) {
            vmData.sendNameUser(etName)
            replaceFragmentWithBackStack(fragmentContainer, FragmentStudentConfirmation(fragmentContainer, toastContainer), null)
        } else {
